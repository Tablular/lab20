type image = float list list ;;
(* images are lists of lists of floats between 0. (white) and 1. (black) *)
type size = int * int ;;
open Graphics ;;

let doublemap (i : 'a list list) (f : 'a -> 'b) : 'a list list =
  List.map (fun a -> List.map f a) i ;;

let comparison (cond : float) (input : float): float =
  if input <= cond then 0. else 1. ;;

(* threshold thershold image -- image where pixels above the threshold
value are black *)
let threshold (img : float list list) (threshold : float) : float list list =
  doublemap img (comparison threshold) ;;

(* show the image *)
let depict (img : float list list) : unit =
  Graphics.clear_graph ();
  let x, y = List.length (List.hd img), List.length img in
    Graphics.resize_window x y;
  let depict_pix v r c =
    let lvl = int_of_float (255. *. (1. -. v)) in
      Graphics.set_color (Graphics.rgb lvl lvl lvl);
  plot c (y - r) in
    List.iteri (fun r row -> List.iteri (fun c pix -> depict_pix pix r c) row) img;
  Unix.sleep 2;;

(* dither max image -- dithered image *)
let dither (img : float list list) : float list list =
  doublemap img (fun v -> if v <= Random.float 1. then 0. else 1.) ;;(*
  doublemap img (comparison (Random.float 1.))*)


Graphics.open_graph "" ;;
let mona = Monalisa.image ;;
depict mona ;;

let mona_threshold = threshold mona 0.75 ;;
depict mona_threshold ;;

let mona_dither = dither mona ;;
depict mona_dither ;;
Graphics.close_graph ;;

